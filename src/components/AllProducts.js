    ...
    <div className='flex justify-center items-center'>
      ...
      <div className='w-full h-full p-4 flex flex-col border-l border-gray-300'>
        <div className='w-full flex'>
          ...
          <button className='rounded-r-md bg-black px-3 py-1 text-white'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" class="font-bold w-4 h-4 text-white">
        ...
        <div className='grid lg:grid-cols-4 md:grid-cols-3 gap-3 mt-4'>
          {
            prods.map(el => <Product title={el.title} price={el.price} src={el.src} key={el.id} prod={el} />)
          }
        </div>
      </div>
    </div>
